...
if [[ ! $(cat ${tmpfile} | grep "https://download.jboss.org/jbosstools/updates/requirements/") ]]; then
...
for u in ${URLs}; do
  REQ_NAME=${u##https://download.jboss.org/jbosstools/updates/requirements/}; REQ_NAME=${REQ_NAME%%/*}; # echo $REQ_NAME
  for t in $(find . -name "*.target" | grep -v "/target/"); do
    echo "[INFO] Processing $t (${REQ_NAME}) ..."
    sed -i "s#<repository location=\"https://download.jboss.org/jbosstools/updates/requirements/${REQ_NAME}/.\+\"/>#<repository location=\"${u}\"/>#" $t
  done
done
...
for d in jbosstools jbdevstudio; do
  ...
  # 4. generate p2diffs
  prefix=https://download.jboss.org/jbosstools; if [[ $d == "jbdevstudio" ]]; then prefix="https://devstudio.jboss.com"; fi
